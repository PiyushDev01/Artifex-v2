...
const updateStatusDesc = async (orderId, statusDesc) => {
    try {
        // Reference to the "users" collection
        const usersRef = collection(db, "users");
        ...
        for (const userDoc of usersSnapshot.docs) {
            ...
            for (const orderDoc of ordersSnapshot.docs) {
                if (orderDoc.id === orderId) {
                    // Update the status description for the order
                    const orderRef = doc(db, "users", uid, "orders", orderDoc.id);
                    await updateDoc(orderRef, { statusMessage: statusDesc });
                    console.log(`Status description updated for order ${orderId} of user ${uid}`);
                }
            }
        }
    } catch (error) {
        console.error("Error updating status description: ", error);
    }
};

export { updateShippingCharge,  updateStatus, updateStatusDesc };
